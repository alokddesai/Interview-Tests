...
public class PhotoGalleryAdapter extends BaseAdapter {
	...
	public PhotoGalleryAdapter(Activity activity,
			ArrayList<GalleryCategoryDataContainer> data,
			DisplayImageOptions options, ImageView imageView) {
		/********** Take passed values **********/
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			holder.tv_category_title = (TextView) view
					.findViewById(R.id.tv_category_type);
			holder.lv_item_type = (LinearLayout) view
					.findViewById(R.id.lv_image_type);
		...
		if (data.size() <= 0) {
			...
			ArrayList<GalleryDataContainer> dataList = tempValues
					.getGalleryDataContainerList();
			...
			for (final GalleryDataContainer dataItem : dataList) {
				...
				params.setMargins(paddingPixel, paddingPixel, paddingPixel,
						paddingPixel);
				imageView.setLayoutParams(params);
				imageView.setLayoutParams(new LinearLayout.LayoutParams(pixels,
						pixels));
				imageView.setOnClickListener(new View.OnClickListener() {
					@Override
					public void onClick(View v) {
						ImageLoader.getInstance().displayImage(
								Constants.BASE_URL + dataItem.getImgURL(),
								selectedImageView, options);
						Toast.makeText(activity, dataItem.getName(),
				...
				ImageLoader.getInstance().displayImage(
						Constants.BASE_URL + dataItem.getImgURL(), imageView,
						options);
